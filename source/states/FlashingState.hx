...
class FlashingState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!leftState) {
			var back:Bool = controls.BACK || (isTab && MusicBeatState._virtualpad.buttonB.justPressed);
			if (controls.ACCEPT || back || (isTab && MusicBeatState._virtualpad.buttonA.justPressed)) {
				leftState = true;
