...
class GameplayChangersSubstate extends MusicBeatSubstate {
	...
	function getOptions() {
		...
		optionsArray.push(new GameplayOption('Lunatic', 'hard', 'bool', false));
	...
	public function new() {
		...
		bg = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, 0xFFFFA500);
		bg.alpha = 0.5;
		add(bg);
	...
	override function update(elapsed:Float) {
		...
		if (controls.UI_UP_P || (isTab && MusicBeatSubstate._virtualpad.buttonUp.justPressed)) {
			changeSelection(-1);
		}
		if (controls.UI_DOWN_P || (isTab && MusicBeatSubstate._virtualpad.buttonDown.justPressed)) {
			changeSelection(1);
		}
		...
		if (nextAccept <= 0) {
			...
			if (usesCheckbox) {
				if (controls.ACCEPT || (isTab && MusicBeatSubstate._virtualpad.buttonA.justPressed)) {
				...
				if (controls.UI_LEFT || controls.UI_RIGHT || (isTab && MusicBeatSubstate._virtualpad.buttonRight.pressed) || (isTab && MusicBeatSubstate._virtualpad.buttonLeft.pressed)) {
					var pressed = (controls.UI_LEFT_P || controls.UI_RIGHT_P || (isTab && MusicBeatSubstate._virtualpad.buttonRight.justPressed) || (isTab && MusicBeatSubstate._virtualpad.buttonLeft.justPressed));
					if (holdTime > 0.5 || pressed) {
						if (pressed) {
							var add:Dynamic = null;
							if (curOption.type != 'string') {
								add = (controls.UI_LEFT || (isTab && MusicBeatSubstate._virtualpad.buttonLeft.pressed)) ? -curOption.changeValue : curOption.changeValue;
							}
							...
							switch (curOption.type) {
								...
								case 'string':
									...
									if (controls.UI_LEFT_P || (isTab && MusicBeatSubstate._virtualpad.buttonLeft.pressed))
										--num;
									else
										num++;
				...
				} else if (controls.UI_LEFT_R || controls.UI_RIGHT_R || (isTab && MusicBeatSubstate._virtualpad.buttonLeft.justReleased) || (isTab && MusicBeatSubstate._virtualpad.buttonLeft.justReleased)) {
					clearHold();
				}
			}

			if (controls.RESET || (isTab && MusicBeatSubstate._virtualpad.buttonC.justPressed)) {
				for (i in 0...optionsArray.length) {
