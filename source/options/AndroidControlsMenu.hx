...
class AndroidControlsMenu extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		inputvari.setFormat(Language.fonts(), 30, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		spacePozition.setFormat(Language.fonts(), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		shiftPozition.setFormat(Language.fonts(), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		for (touch in FlxG.touches.list){		
			if((controls.UI_LEFT_P || touch.overlaps(leftArrow)) && touch.justPressed)
			{
				changeSelection(-1);
			}
			else if ((controls.UI_RIGHT_P || touch.overlaps(rightArrow)) && touch.justPressed)
			{
				changeSelection(1);
			}
			trackbutton(touch);
