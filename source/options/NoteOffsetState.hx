...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		changeModeText.setFormat(Language.fonts(), 32, FlxColor.WHITE, CENTER);
		...
		Conductor.bpm = 128.0;
		FlxG.sound.playMusic(Paths.music('offsetSong'), ClientPrefs.data.musicVolume, true);
	...
	override public function update(elapsed:Float)
	{
		...
		if(onComboMenu)
		{
			...
			if(controls.RESET || (isTab && MusicBeatState._virtualpad.buttonC.justPressed))
			{
		...
		else
		{
			...
			if(controls.UI_LEFT_R || controls.UI_RIGHT_R || (isTab && MusicBeatState._virtualpad.buttonLeft.pressed) || (isTab && MusicBeatState._virtualpad.buttonRight.pressed)) holdTime = 0;
		...
		(controls.controllerMode && FlxG.gamepads.anyJustPressed(START)) || (isTab && MusicBeatState._virtualpad.buttonA.justPressed))
		{
		...
		if(controls.BACK || (isTab && MusicBeatState._virtualpad.buttonB.justPressed))
		{
			...
			if(OptionsState.onPlayState)
			{
				if(ClientPrefs.data.pauseMusic != 'None')
					FlxG.sound.playMusic(Paths.music(Paths.formatToSongPath(ClientPrefs.data.pauseMusic)), ClientPrefs.data.musicVolume);
				else
					FlxG.sound.music.volume = 0;
			}
			else FlxG.sound.playMusic(Paths.music('freakyMenu'), ClientPrefs.data.musicVolume);
	...
	function createTexts()
	{
		for (i in 0...4)
		{
			...
			text.setFormat(Language.fonts(), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch(i)
			{
				case 0: dumbTexts.members[i].text = '${language.States.options.ADAC.ro}';
				case 1: dumbTexts.members[i].text = '[' + ClientPrefs.data.comboOffset[0] + ', ' + ClientPrefs.data.comboOffset[1] + ']';
				case 2: dumbTexts.members[i].text = '${language.States.options.ADAC.no}';
	...
	function updateNoteDelay()
	{
		...
		timeTxt.text = '${language.States.options.ADAC.co}${Math.floor(barPercent)}${language.States.options.ADAC.ms}';
	}
	...
	function updateMode()
	{
		...
		var str2:String;
		if(onComboMenu)
			str = '${language.States.options.ADAC.opt1}';
		else
			str = '${language.States.options.ADAC.opt2}';

		if(!controls.controllerMode)
			str2 = '(${language.States.options.ADAC.pats[0]})';
		else
			str2 = '(${language.States.options.ADAC.pats[1]})';
