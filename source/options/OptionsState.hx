...
class OptionsState extends MusicBeatState {
	...
	override function create() {
		...
		saveText = new FlxText(10, 10);
		saveText.setFormat(Language.fonts(), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		saveText.borderSize = 2;
		saveText.text = "Saved Data!";
		...
		options = [
			...
			new OptionCata(640, 40, "Visuals & UI", [
				...
				new FPSCounter("Display FPSCounter"),
			...
			new OptionCata(935, 40, "Advanced", [
				...
				new SelectPlay("Play Select Song in Fraapley"),
				new ED("CLEAR ALL PLAYER DATA (If you know what you're doing!!)")
			]),
			new OptionCata(50, 40+64, "Debug", [
				new UOR("Restore settings that aren't in use or hidden"),
			...
			new OptionCata(640, 40+64, "Extra Setting", [
				new TM("Tablet mode (beta) is enabled, which is enabled by default on Android"),
				new ZoomMode("Change the lens zoom mode (*Universal)"),
				new SM("Whether to use the system default mouse"),
				new SpaceLocat("Change the position of the touchbar space expansion key")
			]),
			new OptionCata(935, 40+64, "Other Settings", [
				...
				new AndContorls("Enter the \"Android Contorls\" settings interface"),
	...
	override function update(elapsed:Float) {
		...
		if (isInCat) {
			...
			if (selectedOption.acceptType || !selectedOption.acceptType){
				...
				if (right || right_hold) {
					if(selectedOption.onEnable()) {
						if (!right_hold) FlxG.sound.play(PathsList.themeSound('scrollMenu'), ClientPrefs.data.soundVolume);
						var object = selectedCat.optionObjects.members[selectedOptionIndex];
					...
					if(selectedOption.onEnable()) {
						if (!left_hold) FlxG.sound.play(PathsList.themeSound('scrollMenu'), ClientPrefs.data.soundVolume);
						var object = selectedCat.optionObjects.members[selectedOptionIndex];
						selectedOption.left();
						ClientPrefs.saveSettings();
						object.text = selectedOption.getValue();
